...
class VSSEnvGoTo(VSSBaseEnv):
    ...
    def _calculate_reward_and_done(self):
        reward = self.__dist_reward()
        # print(reward)
        if self.reward_shaping_total is None:
            self.reward_shaping_total = {'dist_reward': 0,
                                         'goal_achievement': 0,
                                         'goal_not_achieved': 0,}
    ...
    def _get_initial_positions_frame(self):
        ...
        pos_frame.ball = Ball(x=self.target_point[0], y=self.target_point[1])
    ...
    def __dist_reward(self):
        ...
        distance_to_target = np.sqrt((x_robot - x_target)**2 + (y_robot - y_target)**2)

        # Use the negative distance as the reward to incentivize minimizing the distance
