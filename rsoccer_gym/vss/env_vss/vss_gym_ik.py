...
class VSSEnvIK(VSSBaseEnv):
    ...
    def smallestAngleDiffRad(self, a, b):
        """
        Calculates the smallest difference between two angles a and b (both in radians).
        ...
        Returns:
        - float: The smallest difference between a and b in radians
        """

        result = (b - a + math.pi) % (2 * math.pi) - math.pi
        return abs(result) if abs(result) < math.pi else 2 * math.pi - abs(result)

    def inverse_knematics(self, target_x, target_y, target_theta):
        pos_x = self.frame.robots_blue[0].x
        pos_y = self.frame.robots_blue[0].y
        target_x = target_x * self.field.length / 2
        target_y = target_y * self.field.width / 2
        robot_angle = self.frame.robots_blue[0].theta
        robot_rear_angle = robot_angle + math.pi
        rear = False
        max_v = 40
        max_w = 0.34
        KP = 0.4
        deltaX = target_x - pos_x
        deltaY = target_y - pos_y
        # delta_to_target = math.sqrt(deltaX**2 + deltaY**2)
        desired_angle = math.atan2(deltaY, deltaX)
        angle_error = self.smallestAngleDiffRad(desired_angle, robot_angle)
        rear_angle = self.smallestAngleDiffRad(desired_angle, robot_rear_angle)

        if (abs(rear_angle) < abs(angle_error)):
            rear = True
            angle_error = rear_angle

        angular_velocity = np.clip(KP * angle_error, -max_w, max_w)

        linear_velocity = (1 - abs(angle_error) / math.pi) * max_v
        left_wheel_speed = linear_velocity - \
            (angular_velocity * self.field.rbt_wheel_radius)
    ...
    def _get_commands(self, actions):
        ...
        point_x, point_y = actions

        self.actions[0] = actions
        v_wheel0, v_wheel1 = self.inverse_knematics(point_x, point_y, 0)
