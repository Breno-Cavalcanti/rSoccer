...
class VSSStackEnv(VSSBaseEnv):
    ...
    def __init__(self):
        super().__init__(field_type=0, n_robots_blue=3, n_robots_yellow=3,
                         time_step=0.025, window_size=10)

        self.action_space = gym.spaces.Box(low=-1, high=1,
                                           shape=(2, ), dtype=np.float32)
        self.observation_space = gym.spaces.Box(low=-self.NORM_BOUNDS,
                                                high=self.NORM_BOUNDS,
                                                shape=(10, 40), dtype=np.float32)
        ...
        self.stack_obs = [np.zeros(40) for _ in range(self.window_size)]
    ...
    def _frame_to_observations(self):
        ...
        # ipdb.set_trace()
        self.stack_obs.pop()
        self.stack_obs.insert(0, observation)

        return np.array(self.stack_obs, dtype=np.float32)
